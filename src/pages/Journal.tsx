                ...
                {extraProducts.map(p => (
                  <li key={p.label}>
                    <button className="group w-full flex items-center gap-3 px-3 py-2 rounded-xl text-muted-foreground hover:text-foreground transition-all relative">
                      ...
                      <span className={`relative z-10 text-[9px] font-bold px-1.5 py-0.5 rounded-full border flex-shrink-0 ${p.badge==="Included" ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-500/25" : p.badge==="Coming Soon" ? "bg-muted text-muted-foreground border-border" : "bg-primary/10 text-primary border-primary/25"}`}>{p.badge}</span>
                    </button>
                  </li>
                ))}
              </ul>
